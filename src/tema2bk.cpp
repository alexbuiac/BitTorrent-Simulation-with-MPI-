    ...
    for (string filename : data->files_wanted) {
        ...
        for (int i = 0; i < num_chunks; i++) {
            ...
            while (!data->downloaded_chunks[filename][i]) {
                ...
                filename_c[filename.length()] = '\0';
                
                // send request
                int req = 1;
                MPI_Send(&req, 1, MPI_INT, peers[peer_index], 17, MPI_COMM_WORLD);
                MPI_Send(filename_c, MAX_FILENAME, MPI_CHAR, peers[peer_index], 20, MPI_COMM_WORLD);
...
int check_chunk(string &filename, string &hash, in_data *data) {
    for (auto &it : data->hashes[filename]) {
        if (it == hash) {
            return 1;
        }
    }
...
void *upload_thread_func(void *arg)
{
    ...
    while (true) {
        ...
        pthread_mutex_lock(&data->mutex);
        int answer = check_chunk(filename, hash, data);
        pthread_mutex_unlock(&data->mutex);
...
void tracker(int numtasks, int rank) {
    ...
    for (int i = 1; i < numtasks; i++) {
        ...
        for (int j = 0; j < num_files_had; j++) {
            ...
            // if we don't have the file, recv chunks
            int num_chunks;
